        ...
        await Transaction.findOne({tid: param.id},async (err,transaction)=>{
            ...
            if(transaction.state === 1){
                ...
                const journal = new Journal({
                    ...
                    //     let temp = [];
                    //     param.size.forEach(key=>{
                    //         if(key.size == element.ss){
                    //             temp.push({size: key.size, status: key.status, num: parseInt(key.num) - parseInt(element.num)})
                    //         } else {
                    //             temp.push(key)
                    //         }
                    //     })
                    //     param.size = temp;
                    //     await Param.findByIdAndUpdate({_id: element.param}, {$set: param});
                    // })
...
exports.paymeUrl = async (req,res)=>{
    // const PAYCOM_PASSWORD = 'js9E6CNWHCRG9%@3&IB?9Ao8@jHU2?4TyNOy'; //test
    const PAYCOM_PASSWORD = 'h%Z7wi%PY7k1qWYbJCvexGu&B0iwqGIZGHFS'; //prod
    ...
    async function CheckPerformTransaction (id,amount){
        await Buy.findOne({order: id},(err,data)=>{
            if(err || !data ) return sendResponse(BillingErrors.OrderNotFound(),null);
            if(data.payed === 1) return sendResponse(BillingErrors.OrderAvailable(),null);
            if(data.amount !== amount / 100)  return sendResponse(BillingErrors.IncorrectAmount(),null);
            return  sendResponse(null,{
    ...
    async function CreateTransaction(param){
        await Transaction.findOne({buy: param.account.order},async (err,data)=> {
            if(!data){
                await Buy.findOne({order: param.account.order},async (err,order)=>{
                    if(err || !order ) return sendResponse(BillingErrors.OrderNotFound(),null);
                    if(order.payed === 1) {
                        return sendResponse(BillingErrors.OrderAvailable(), null);
                    }
                    if(order.amount !== param.amount / 100){
                        ...
                        const transaction = new Transaction({
                            ...
                            cancel_time:0,
                            create_time: Date.now(),
                            buy: parseInt(param.account.order),
                            time: param.time
                        })
                        transaction.save()
                            .then(()=>{
    ...
    async function PerformTransaction(param){
        await Transaction.findOne({tid: param.id},async (err,transaction)=>{
            ...
            if(transaction.state === 1){
                ...
                const order = await Buy.findOne({order: transaction.buy})
                const journal = new BuyJournal({
                    system: "payme",
                    amount: transaction.amount,
                    order: order._id
                })
                ...
                await Buy.updateOne({order: transaction.buy},{
                    $set: {
                        payed: 1,
                        paySystem: 'payme'
                    }
    ...
    async function CancelTransaction(param){
        await Transaction.findOne({tid: param.id},async (err,transaction)=>{
            ...
            if(transaction.state === 1){
                ...
                await Transaction.findOne({tid: transaction.tid},async (err,data)=>{
                    await Buy.updateOne({order: transaction.order},{$set:{
                            payed: 0
                        }},async (err,orr)=>{
                        if(err) return sendResponse(err,null)
                        const ord = await Buy.find({order: transaction.order})
                        await BuyJournal.findOneAndDelete({order: ord._id},(err,data)=>{
                            if(err) return sendResponse(err,null)
                        })
                        await Buy.findOneAndDelete({_id: ord._id},(err,data)=>{
                            if(err) return sendResponse(err,null)
                        })
                        return sendResponse(null,{
                            state: data.state,
                ...
                if(transaction.state === 2){
                    await Buy.findOne({order: transaction.order},async (err,order)=>{
                        if(err) return sendResponse(err,null);
                        if(order.payed === 0){
                            await Transaction.updateOne({tid: param.id},{$set:{
                                    state: -2,
